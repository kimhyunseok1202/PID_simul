...
비례 게인 Kc (Proportional): 현재 오차에 즉각 반응하는 세기. 너무 크면 진동이 심해집니다.

적분 시간 τI (Integral): 잔류 오차를 누적 계산해 제거. 작을수록 적분 작용이 강해집니다.

미분 시간 τD (Derivative): 급격한 온도 변화율을 억제하는 브레이크 역할입니다.
""")
...
fig, axes = plt.subplots(active_plots, 1, figsize=(10, 4.5 * active_plots), squeeze=False, sharex=True)
current_ax_idx = 0

if show_temp_graph:
    ax_temp = axes[current_ax_idx, 0]
    ax_temp.plot(time, T, "m-", linewidth=2, label=f"PID 제어 온도 (최종값: {T[-1]:.2f}℃)")
    ax_temp.axhline(setpoint, color="r", linestyle="--", linewidth=1.2, label=f"목표치 ({setpoint}℃)")
